...
class Node:
    ...
    def __init__(self, value=None, left=None, right=None, wholeSet=None):
        self.value = value
        self.left = left
        self.right = right
        self.wholeSet = wholeSet
...
class SyntaxTree:
    ...
    def conjunction(self, leftResult: set, rightResult: set, wholeSet: set):
        """Conjunction of 2 sets"""
        inter = []
        for elt in leftResult:
            if elt in rightResult:
                inter.append(elt)
        return set(inter), wholeSet

    def disjunction(self, leftResult: set, rightResult: set, wholeSet: set):
        """Disjunction of 2 sets"""
        union = list(leftResult)
        for elt in rightResult:
            union.append(elt)
        return set(union), wholeSet

    def negation(self, leftResult: set, wholeSet: set):
        """Negation of a set"""
        neg = wholeSet.copy()
        for elt in leftResult:
            neg.discard(elt)
    ...
    def evaluateSets(self, root: Node):
        """
        a function that takes as input the root of a syntax tree,
        made of sets, evaluates this tree,
        then returns the result.
        NB : as remarks,
             a simpler algorithm with build-in python set expressions
        """
        if root is None:  # empty tree
            return
        if root.left is None and root.right is None:  # leaf
            return root.value, root.wholeSet
        lResult = self.evaluateSets(root.left)  # evaluate left tree
        rResult = self.evaluateSets(root.right)  # evaluate right tree

        if root.value == "&":
            return self.conjunction(lResult[0], rResult[0], lResult[1])
            # return leftResult[0] & rightResult[0], leftResult[1]
        elif root.value == "|":
            return self.disjunction(lResult[0], rResult[0], lResult[1])
            # return leftResult[0] | rightResult[0], leftResult[1]
        elif root.value == "!":
            return self.negation(lResult[0], lResult[1])
            # return leftResult[1] - leftResult[0], leftResult[1]


def errorMessage():
    """ guess what it does"""
    print("syntax error, missing or wrong elements")
    return None
...
def postfixToSyntaxTree(formula: str, setDict=None, wholeSet=None) -> Node:
    """
    a function that takes as input a string that contains
    a propositional formula in reverse polish notation,
    then returns the corresponding syntax tree of sets
    """
    stack = deque()
    for letter in formula:
        if letter in BINARY_OP:
            ...
            if letter.isupper():  # or letter in ["0", "1"]:
                if setDict and letter in setDict.keys():
                    letter = setDict[letter]
                stack.append(Node(letter, None, None, wholeSet))
            else:
                return errorMessage()
    if len(stack) != 1:
        return errorMessage()
...
def nnfForCnf(formula: str) -> Node:
    """
    Same function as negation_normal_form, but returns
