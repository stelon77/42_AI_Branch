...
class MyLinearRegression:
    ...
    def fit_(self, x, y):
        ...
        """
        if not MyLinearRegression. \
           is_two_vector_not_empty_same_size_numpy(x, y):
            return None
        x = MyLinearRegression.reshape_if_needed(x)
        y = MyLinearRegression.reshape_if_needed(y)
        if not MyLinearRegression. \
           is_not_empty_numpy_vector_2_1_of_numbers(self.thetas):
            return None
    ...
    def loss_elem_(self, y, y_hat):
        ...
        """
        if not MyLinearRegression. \
           is_two_vector_not_empty_same_size_numpy(y, y_hat):
            return None
    ...
    def mae_elem_(self, y, y_hat):
        ...
        y = MyLinearRegression.reshape_if_needed(y)
        dif = y_hat - y
        return(np.absolute(dif))

    def r2score_elem_(self, y, y_hat):
        """
        Description:
        ...
        y_mean = np.mean(y)
        dif2 = y - y_mean
        return (np.square(dif), np.square(dif2))
    ...
    def rmse_(self, y, y_hat):
        """
        Description:
        Calculates the value of rmse loss function.
        Args:
        y: has to be an numpy.array, a vector.
        y_hat: has to be an numpy.array, a vector.
        Returns:
        ...
        This function should not raise any Exception.
        """
        return (math.sqrt(MyLinearRegression.mse_(self, y, y_hat)))

    def mae_(self, y, y_hat):
        """
        Description:
        Calculates the value of mae loss function.
        Args:
        y: has to be an numpy.array, a vector.
        y_hat: has to be an numpy.array, a vector.
        Returns:
        ...
        This function should not raise any Exception.
        """
        J_elem = MyLinearRegression.mae_elem_(self, y, y_hat)
        if J_elem is None or len(J_elem) == 0:
            return None
    ...
    def r2score_(self, y, y_hat):
        """
        Description:
        Calculates the value of r2score loss function.
        Args:
        y: has to be a numpy.array, a vector of shape m * 1.
        ...
        This function should not raise any Exception.
        """
        J_elem_num, J_elem_denom = MyLinearRegression. \
            r2score_elem_(self, y, y_hat)
        if J_elem_num is None or J_elem_denom is None:
            return None
        denominator = np.sum(J_elem_denom)
        if denominator == 0:
            return None
        J_value = 1 - (np.sum(J_elem_num) / denominator)
        return float(J_value)

    # ---------------DISPLAY FUNCTIONS ____________________________ #

    def data_hypothesis_graph(self, x, y, y_hat):
        plt.scatter(x, y, marker='o', color='deepskyblue',
                    label=r"$S_{true}(pills)$")
        plt.plot(x, y_hat, '--', color='limegreen', marker='o',
                 label=r"$S_{predict}(pills)$")
        leg = plt.legend(bbox_to_anchor=(0, 1.001, 1, 0.001),
    ...
    def cost_function_graph(self, x, y):
        colors = ['black', 'dimgrey', 'grey', 'darkgrey',
                  'silver', 'lightgrey']
        n = 0
        for theta0 in range(77, 100, 4):
            thetas1 = np.linspace(-14, -4, 256, endpoint=True)
            loss = []
            for theta1 in thetas1:
                y_hat = (x * theta1) + theta0
                mse = MyLinearRegression.mse_(self, y, y_hat)
                loss.append(mse)
            cost = np.array(loss)
            plt.plot(thetas1, cost, label=r"$J({\theta}_{0} = $" +
                     str(theta0) + r"$,{\theta}_{1})$", color=colors[n])
            n += 1
            if n > 6:
                n = 6
        ...
        plt.ylabel(r"$cost\ function\ J({\theta}_{0},{\theta}_{1})$")
        plt.grid()
        plt.show()
    ...
    @staticmethod
    def is_not_empty_numpy_vector_2_1_of_numbers(x):
        if MyLinearRegression.is_numpy_array(x) and \
           MyLinearRegression.is_not_empty(x) and \
           MyLinearRegression.is_made_of_numbers(x) and \
