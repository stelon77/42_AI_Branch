...
class MyLinearRegression:
    ...
    def __init__(self, theta, alpha=0.001, max_iter=1000):
        if not isinstance(alpha, (float, int)) or \
           ...
           not isinstance(theta, (np.ndarray, list, tuple)) or \
           not self.is_made_of_numbers(theta):
    ...
    def gradient(self, x, y, theta):  # used in fit, theta not from self
    ...
    def gradient_(self, x, y):
        """Computes a gradient vector from 2
        non-empty numpy.array, without any for-loop.
        ...
        X = MyLinearRegression.add_intercept(x)
        m = x.shape[0]
        return np.dot(np.transpose(X), (np.dot(X, self.theta) - y)) / m
